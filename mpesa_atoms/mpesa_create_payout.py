...
def mpesa_create_payout(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Business-to-Customer (B2C): send money from a business shortcode to a customer M-Pesa wallet.
    Official: https://developer.safaricom.co.ke/apis/BusinessToCustomer

    payload:
      amount (required), phone_number / PartyB (required, recipient MSISDN),
      command_id: BusinessPayment | SalaryPayment | PromotionPayment (default BusinessPayment),
      remarks, occasion, result_url / ResultURL (required),
      queue_timeout_url / QueueTimeOutURL (required), originator_conversation_id (optional, auto-generated)

    auth_info: business_short_code (PartyA), initiator_name, security_credential
    ...
    try:
        ...
        body_payload, err = _mp_b2c_fields(auth_info, payload)
        if err:
            return _mp_provision({}, 400, err)
        resp, body, status, msg = _mp_post("/mpesa/b2c/v1/paymentrequest", base_url, auth_info, body_payload, timeout, verify_ssl)
        if status >= 400:
...
def _mp_provision(data, status, msg, fallback_id=None):
    ...
    pid = obj.get("ConversationID") or obj.get("OriginatorConversationID") or obj.get("CheckoutRequestID") or fallback_id
...
def _mp_b2c_fields(auth_info, payload):
    auth_info = auth_info or {}
    payload = payload if isinstance(payload, dict) else {}
    shortcode = payload.get("PartyA") or auth_info.get("business_short_code") or auth_info.get("shortcode")
    initiator = payload.get("InitiatorName") or auth_info.get("initiator_name")
    if not shortcode:
        return None, "business_short_code (PartyA) is required"
    if not initiator:
    ...
    phone = _mp_phone(payload.get("PartyB") or payload.get("phone_number") or payload.get("recipient"))
    if not phone:
        return None, "phone_number (PartyB) is required"
    ...
    return {
        "OriginatorConversationID": payload.get("OriginatorConversationID") or payload.get("originator_conversation_id") or _mp_uuid(),
        "InitiatorName": str(initiator),
        "SecurityCredential": cred,
        "CommandID": payload.get("CommandID") or payload.get("command_id") or "BusinessPayment",
        "Amount": amount,
        "PartyA": str(shortcode),
        "PartyB": phone,
        "Remarks": str(payload.get("Remarks") or payload.get("remarks") or "Payout")[:100],
        "QueueTimeOutURL": timeout_url,
        "ResultURL": result_url,
        "Occasion": str(payload.get("Occasion") or payload.get("occasion") or "")[:100],
    }, None
