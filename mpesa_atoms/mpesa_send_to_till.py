...
def mpesa_send_to_till(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Business Buy Goods (B2B): pay a Till/Buy-Goods number from your own business shortcode.
    Official: https://developer.safaricom.co.ke/apis/BusinessBuyGoods

    payload:
      amount (required), till_number / PartyB (required, receiving Till),
      account_reference, remarks, result_url / ResultURL (required),
      queue_timeout_url / QueueTimeOutURL (required), originator_conversation_id (optional)
    ...
    try:
        ...
        body_payload, err = _mp_b2b_fields(auth_info, payload, command_id="BusinessBuyGoods", receiver_identifier_type="2")
        if err:
...
def _mp_b2b_fields(auth_info, payload, command_id, receiver_identifier_type):
    ...
    return {
        ...
        "PartyB": receiver,
        "AccountReference": str(payload.get("AccountReference") or payload.get("account_reference") or "payment")[:12],
